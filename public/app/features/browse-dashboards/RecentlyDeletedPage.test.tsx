...
jest.mock('./components/SearchView', () => ({ SearchView: () => null }));
jest.mock('./components/RecentlyDeletedActions', () => ({ RecentlyDeletedActions: () => null }));
jest.mock('./components/RecentlyDeletedEmptyState', () => ({ RecentlyDeletedEmptyState: () => null }));
jest.mock('app/features/search/page/components/ActionRow', () => ({ ActionRow: () => null }));
...
describe('RecentlyDeletedPage banner integration', () => {
  ...
  it('renders the nearing banner at the threshold boundary', async () => {
    config.featureToggles.restoreDashboards = true;
    mockGetAsResourceList.mockResolvedValue(buildList(850));
